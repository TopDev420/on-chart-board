<?php namespace App\Modules\Payeer\Controllers\Admin;
...
class Payment_gateway extends BaseController
{
	...
	{ 
		$pagewhere=array('identity'=>'payeer');
        ...
        if ($this->request->getMethod() == 'post') {
			...
			{	
	            ...
	            return redirect()->to(base_url('backend/payment_gateway/payeer_setting'));

			} else {

				$this->session->setFlashdata("exception", $this->validation->listErrors());
	            return redirect()->route('backend/payment_gateway/payeer_setting');
			}
		}
