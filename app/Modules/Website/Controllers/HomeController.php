...
class HomeController extends BaseController
{
    ...
    public function tradehistory()
    {
        ...
        $coinhistory        = $this->db->table('dbt_coinhistory')->select('*')->where('market_symbol', $market_symbol)->orderBy('date', 'desc')->get()->getResult();
        ...
        $coinhistory    = $this->db->table('dbt_coinhistory_detail')->selectMin('price', 'low')->selectMax('price', "high")->selectMin('date', 'open_date')->selectMax('date', 'close_date')->select('substring_index(group_concat(price order by date), trim(","), 1 ) as "open"')->select('substring_index(group_concat(price order by date desc), trim(","), 1 ) as "close"')->select('FLOOR(UNIX_TIMESTAMP(date)/' . $interval . ') as "timekey"')->where('market_symbol', $market_symbol)->groupBy('timekey')->orderBy('id', 'asc')->limit($limit, 0)->get()->getResult();
