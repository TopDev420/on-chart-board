<?php 
namespace App\Modules\Stripe\Controllers\Admin;
...
class Payment_gateway extends BaseController
{
	...
	{ 
		$pagewhere=array('identity'=>'stripe');
        ...
        if ($this->request->getMethod() == 'post') {
			...
			{	
	            ...
	            return redirect()->to(base_url('backend/payment_gateway/stripe_setting'));

			} else {

				$this->session->setFlashdata("exception", $this->validation->listErrors());
				return redirect()->route('backend/payment_gateway/stripe_setting');
			}
		}
